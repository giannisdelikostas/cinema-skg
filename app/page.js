...
export default function CinemaApp() {
  ...
  const navigateModal = (direction) => {
    const list = isComingSoon ? upcomingMovies : filteredMovies;
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-10 font-inter relative overflow-x-hidden selection:bg-yellow-500 selection:text-black">
      <style jsx global>{`
        ...
        body { font-family: 'Inter', sans-serif; letter-spacing: -0.02em; scroll-behavior: smooth; }
      ...
      <header className="max-w-3xl mx-auto py-16 md:py-24 border-b border-white/5 mb-12 text-center relative z-10">
        <h1 className="text-6xl md:text-8xl font-[900] text-yellow-500 italic tracking-tighter mb-4 uppercase">{cleanGreek('Cinema SKG')}</h1>
        <p className="text-white/40 text-[10px] md:text-[12px] tracking-[0.4em] font-bold mb-2 uppercase">{cleanGreek('Ολη η Θεσσαλονικη στη μεγαλη οθονη')}</p>
      ...
      <div className="max-w-4xl mx-auto mb-10 relative z-10">
        <div className="flex overflow-x-auto gap-3 pb-4 no-scrollbar items-center">
          ...
          <a href="#coming-soon" className="px-6 py-4 rounded-[1.5rem] text-[11px] font-[900] bg-red-600 hover:bg-red-500 text-white transition-all whitespace-nowrap flex-shrink-0 flex items-center gap-2 animate-pulse shadow-[0_0_20px_rgba(220,38,38,0.4)] uppercase tracking-widest italic border border-transparent">
            🔥 HOT ΠΡΟΣΕΧΩΣ
          </a>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto space-y-8 relative z-10">
        {loading ? (
          <div className="flex justify-center py-24 opacity-40 text-[11px] font-black tracking-[0.5em] animate-pulse uppercase">{cleanGreek('Συγχρονισμος...')}</div>
      ...
      <ComingSoon onOpenModal={openComingSoonModal} />

      <footer className="max-w-3xl mx-auto py-32 text-center relative z-10">
        <div className="opacity-20 text-[10px] font-black tracking-[0.6em] mb-6 uppercase">© {new Date().getFullYear()} {cleanGreek('Cinema SKG')}</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/95 backdrop-blur-xl animate-in fade-in duration-300" onClick={() => setIsModalOpen(false)}>
          <button onClick={(e) => { e.stopPropagation(); navigateModal(-1); }} className="fixed left-4 md:left-10 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-yellow-500 hover:text-black transition-all z-[110] cursor-pointer shadow-2xl backdrop-blur-md">←</button>
          <button onClick={(e) => { e.stopPropagation(); navigateModal(1); }} className="fixed right-4 md:right-10 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-yellow-500 hover:text-black transition-all z-[110] cursor-pointer shadow-2xl backdrop-blur-md">→</button>
          ...
          <div className="flex min-h-full items-center justify-center p-4 md:p-6 cursor-default">
            <div className="bg-[#0a0a0a] border border-white/10 rounded-[3rem] p-8 md:p-12 max-w-lg w-full relative shadow-2xl my-8 overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex flex-col items-center mb-8 text-center uppercase">
                {!isComingSoon ? (
                  <>
                  ...
                  <>
                    <p className="text-yellow-500 font-[900] tracking-[0.2em] italic text-[10px] leading-none mb-2 uppercase">Ημερομηνια Πρεμιερας</p>
                    <p className="text-white font-[900] text-3xl tracking-tighter leading-none italic">{upcomingMovies[currentIndex]?.date}</p>
                  </>
                )}
              </div>
              ...
              <div className="w-full max-w-[260px] mx-auto mb-8 rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl aspect-[2/3] relative">
                  <img src={isComingSoon ? tmdbInfo?.poster || upcomingMovies[currentIndex]?.poster : filteredMovies[currentIndex]?.image} alt="movie poster" className="absolute inset-0 w-full h-full object-cover"/>
              </div>

              <h2 className="text-3xl md:text-4xl font-[900] text-yellow-500 mb-10 leading-tight tracking-tight italic text-center mx-auto max-w-[90%]">{isComingSoon ? upcomingMovies[currentIndex]?.title : filteredMovies[currentIndex]?.title}</h2>
              
              {modalLoading ? (
                <div className="py-24 text-center text-white/20 animate-pulse text-[11px] font-black tracking-[0.5em] uppercase">{cleanGreek('Φορτωση...')}</div>
              ) : tmdbInfo?.error ? (
                <div className="py-24 text-center text-red-400/40 text-[11px] font-black tracking-[0.5em] border border-red-500/10 rounded-[2rem] uppercase">{cleanGreek('Δεν βρεθηκαν πληροφοριες')}</div>
              ) : tmdbInfo ? (
